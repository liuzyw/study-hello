...
public class PrintMatirx {
    ...
    public static void printMatrixClockWisely1(int[][] numbers) {
        // 输入的参数不能为空
        if (numbers == null) {
            return;
        }
    ...
    private static void printMatrixClockWisely(int[][] arr) {
        ...
        while ((rowB <= rowE || colB <= colE)) {
            boolean flag = false;
            for (int k = colB; k <= colE; k++) {
                System.out.print(arr[rowB][k] + "  ");
                flag = true;
            }
            ...
            for (int k = rowE; k >= rowB; k--) {
                System.out.print(arr[k][colB] + "  ");
                flag = true;
            }
            if (flag) {
                colB++;
